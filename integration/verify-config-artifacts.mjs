...
// node integration/verify-config-artifacts.mjs <fresh-artifact-root> [SDK-feed]
...
for (const [index, side] of ['client', 'server', 'client', 'server'].entries()) {
  ...
  const expected = ['manifest.json', ...files(path.join(root, 'config', side)).map(x => `${side}/${x}`)].sort();
  assert.deepEqual(files(config), expected);
  for (const file of expected) assert.equal(hash(path.join(config, file)), hash(path.join(root, 'config', file)), file);
