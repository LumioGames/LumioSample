...
| `LUMIO_TEST_VOXEL_FIXTURE_DIR` | 发布物随带的 `Engine/tools/fixtures/catalog-world/`，须与发布物 native 同源 |
...
取发布物 native 与 catalog world，缺了是 `ENGINE_RELEASE_INPUT_MISSING` 失败。
...
不检出任何私有仓，也不再现编 HostEntry 或 voxel fixture（ADR-123）。
