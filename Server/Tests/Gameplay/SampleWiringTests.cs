...
public sealed class SampleWiringTests : IDisposable
{
    ...
    {
        _repoConfig = Path.GetFullPath(Path.Combine(AppContext.BaseDirectory, "..", "..", "..", "..", "..", "Server", "Config", "Tables"));
        PointAt(_repoConfig);
    }
...
internal sealed class TempConfig : IDisposable
{
    ...
    public static TempConfig WithHits(int hits)
    {
        ...
        foreach (string file in System.IO.Directory.GetFiles(Path.Combine(repo, "Gameplay", "Tables"), "*", SearchOption.AllDirectories))
        {
            string target = Path.Combine(source, Path.GetRelativePath(Path.Combine(repo, "Gameplay", "Tables"), file));
