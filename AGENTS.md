...
收口门槛：`dotnet build LumioSample.slnx`；`dotnet test LumioSample.slnx -- --minimum-expected-tests 1`；`node --test Tools/verify-evidence.mjs`（显式测试文件，至少 1 个测试）；`node .spec/tools/spec-lint.mjs`。
