 ...
 * restores Server/Assets/Maps/sample.voxel from LUMIO_BASE_MAP_PATH; this gate fails closed
...
export function baseMapCaptureAgreement({ root = ROOT, dsConfig, env = {} } = {}) {
  const mapPath = resolve(String(env.LUMIO_BASE_MAP_PATH || join(root, 'Server', 'Assets', 'Maps', 'sample.voxel')));
  if (!isFilePath(mapPath)) {
    return {
      ok: false,
      reason: `first-boot voxel capture is not a file: ${mapPath} (empty TryAttach leaves avatars at 16.5; DS boot must restore Server/Assets/Maps/sample.voxel)`,
      path: mapPath,
    };
  }
...
function buildChildEnv({ env = process.env, root = ROOT, dsConfig, engineNative } = {}) {
  ...
  // Mirrors server.json#config_dir: the DS loads the server end's S+V tables.
  if (!output.LUMIO_CONFIG_DIR) output.LUMIO_CONFIG_DIR = resolve(root, 'Server', 'Config', 'Tables');
  // Bots and the spectator are clients. split-export/1 keeps the C projection in
  // its own tree, and the server end carries none of it, so they cannot reuse
  // LUMIO_CONFIG_DIR any more.
  ...
  try {
    ...
    const spawnBotChild = async (index, ticketsPath, dirSuffix = '') => {
      ...
      const args = buildBotArgs({
        ...
        configDir: options.configDir == null ? childEnv.LUMIO_CLIENT_CONFIG_DIR : resolve(root, options.configDir),
        logDir,
...
export function usage() {
  return [
    'Usage: node Tools/spectator-100.mjs --bots 100 [--authorize-live] [--start-platform] [options]',
