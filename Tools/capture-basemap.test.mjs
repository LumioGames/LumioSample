...
test('committed sample.voxel is a restoreable VoxelEngine capture', () => {
  ...
  const bytes = readFileSync(new URL('../Server/Assets/Maps/sample.voxel', import.meta.url));
...
test('gameplay and DS sources do not import the layout script', () => {
  const gameplayRoot = resolve(ROOT, 'Gameplay');
  const hits = [];
  const walk = (dir) => {
  ...
  const dsConfig = readFileSync(new URL('../Server/Config/Startup/server.json', import.meta.url), 'utf8');
