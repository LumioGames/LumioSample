...
5. `TreatWarningsAsErrors` 在部分 SDK 宿主上仍会把 CS1591 抬成错误，即使玩法 csproj 已 `NoWarn`。sibling / nuget 共用的补救写在 `Directory.Build.targets`：再声明一次 `NoWarn` + `WarningsNotAsErrors`。生成物不得手补 XML 文档。
