...
| `SpectatorDump.cs`, `SpectatorReplicaHost.cs` (bound to the Sample registry and config) | `ds-close-codes.mjs`, `not-serving-retry.mjs` (generated by the engine) |
| `host/` wasm host, `tests/` C# tests, `*.test.mjs` | `replica/netstandard2.1/` (`Lumio.Client.Gameplay.ECS.dll`, `Lumio.Client.Log.dll`, `Lumio.Client.Spectator.dll` = `SpectatorSectionFrame` + `ReplicaApplyTickLoop`), `lumio_voxel_wasm.wasm` |

ADR-123: `Engine/` is the read-only LumioEngineRelease submodule; nothing is taken from a
LumioClient or LumioVoxelEngine checkout, and the release carries no source. The publish
links the flat `web/*.mjs` parts into `wwwroot/` beside `main.js`, both C# projects
reference `Lumio.Client.Spectator.dll` (never compile engine `.cs` by link), and the node
tests read the same files. An empty `Engine/` is a BLOCKED build or a failing test naming
...
`tests/Lumio.Sample.Client.Spectator.Tests.csproj`, which compiles both game sources
against the release's engine DLLs and this game's client gameplay, so a Sample change that no
...
The browser gameplay build (`netstandard2.1`) restores the Runtime from the release's
`Lumio.Engine.SDK` `lib/netstandard2.1` (the release packs it, R-00785).
