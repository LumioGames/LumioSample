...
| From this repository | From the `Engine/` release `web/` (engine parts, never copied here) |
| --- | --- |
| `index.html`, `main.js`, `spectator.css` | `spectator/connect-ds.mjs`, `spectator/voxel-grid.mjs`, `spectator/dev-hot-reload.mjs` |
| `SpectatorDump.cs`, `SpectatorReplicaHost.cs` (bound to the Sample registry and config) | `spectator/ds-close-codes.mjs`, `spectator/not-serving-retry.mjs` (generated by the engine) |
| `host/` wasm host, `tests/` C# tests, `*.test.mjs` | `spectator/cs/SpectatorSectionFrame.cs`, `spectator/cs/ReplicaApplyTickLoop.cs`, `replica/netstandard2.1/` (`Lumio.Client.Gameplay.ECS`, `Lumio.Client.Log`), `lumio_voxel_wasm.wasm` |

ADR-123: `Engine/` is the read-only LumioEngineRelease submodule; nothing is taken from a
LumioClient or LumioVoxelEngine checkout. The publish links the JS parts into `wwwroot/`
beside `main.js`, both C# projects compile the two C# parts by link, and the node tests
read the same files. An empty `Engine/` is a BLOCKED build or a failing test naming
`git submodule update --init --depth 1 Engine`, never a local copy and never a skip.
...
publish copies it in from `Engine/web/lumio_voxel_wasm.wasm` (the release builds it from
VoxelEngine's own Rust) and **fails closed** (`SPECTATOR_VOXEL_WASM_MISSING`) when the
...
and this repository's real `Server/Assets/Maps/sample.voxel` Sections (`voxel-grid.mjs` and the
`.wasm` are read from `Engine/web/`). It fails, never skips, when the inputs are missing:
...
not reach them. What keeps this seam honest is the `test` job in
`.github/workflows/ci.yml`: with `Engine/` fetched by the checkout it builds
`tests/Lumio.Sample.Client.Spectator.Tests.csproj`, which compiles both game sources and
both engine C# parts against this game's client gameplay, so a Sample change that no
longer fits the pinned engine turns the PR red instead of surfacing the next time
...
build, and the release's netstandard2.1 Replica (`Engine/web/replica/netstandard2.1/`).
...
The engine revision is whatever `Engine/` is pinned to; there is no other one to point at.
The browser gameplay build (`netstandard2.1`) needs the release's `Lumio.Engine.SDK` to carry
`lib/netstandard2.1` (requested of the release pipeline, R-00779).
